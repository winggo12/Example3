...
//            let xPos = (previewView.frame.origin.x + (previewView.frame.size.width - overlayView.frame.size.width) / 2);
//            let yPos = (previewView.frame.origin.y + (previewView.frame.size.height - overlayView.frame.size.height) / 2);
//            let width = 0.75*previewView.frame.size.width
//            let height = previewView.frame.size.height
//            overlayView.frame = CGRect(x: xPos, y: yPos, width: width, height: height)
            
        case .landscapeLeft:
            previewView.previewLayer.connection?.videoOrientation = .landscapeRight
//            let xPos = (((previewView.frame.origin.x + (previewView.frame.size.width - overlayView.frame.size.width) / 2))); //(CGFloat)(0);
//            let yPos = (previewView.frame.origin.y + (previewView.frame.size.height - overlayView.frame.size.height) / 2);
//            let width = 0.75*previewView.frame.size.width
//            let height = previewView.frame.size.height
//            overlayView.frame = CGRect(x: xPos, y: yPos, width: width, height: height)
...
extension ViewController: CameraFeedManagerDelegate {
    ...
    func cameraFeedManager(_ manager: CameraFeedManager, didOutput pixelBuffer: CVPixelBuffer) {
        let (result,times) = (thisModel?.Run(pb: pixelBuffer, olv: self.overlayView.frame, pv: self.previewView.frame))!
